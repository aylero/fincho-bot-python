...
class MessageFormatter:
    ...
    def format_status_message(service_data: Dict[str, Any], system_data: Dict[str, Any]) -> str:
        if "error" in service_data:
            service_status = f"⚠️ **Service Unavailable**: {service_data['error']}"
        else:
        ...
        system_status = (
            f"**🖥️ System Status** (`{system_data['system']['hostname']}`)\n"
...
async def help_command(message: types.Message):
    help_text = (
        ...
        "🔹 `/service` - Service information only\n"
...
async def legacy_check_command(message: types.Message):
    await message.answer("ℹ️ This command has been renamed. Please use `/check` instead.")
